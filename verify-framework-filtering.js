// Framework Filtering Verification Script
// This demonstrates the exact filtering logic for the specific combinations mentioned

const { INDUSTRY_FRAMEWORKS, AGENT_TYPE_FRAMEWORK_PREFERENCES } = require('./lib/prompt-templates.ts')
...
testCombinations.forEach(({ agent, industry, description }) => {
  ...
  // Step 5: Final result
  console.log(`\n5. FINAL RESULT:`)
  const finalFrameworks = [...agentFrameworks, ...compatibleFrameworks.slice(0, 4)]
  console.log(`   Total frameworks: ${finalFrameworks.length}`)
  console.log(`   Agent frameworks: ${agentFrameworks.length}`)
  console.log(`   Industry frameworks added: ${compatibleFrameworks.slice(0, 4).length}`)
